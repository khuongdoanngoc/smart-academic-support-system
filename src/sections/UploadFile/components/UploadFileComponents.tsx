    ...
    const onFileDrop = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (newFile) {
            ...
            if (!allowedExtensions.includes(fileExtension || "")) {
                setInformationAlert(
                    "Only docx, pdf, or xlsx files are allowed."
                );
            ...
            if (fileSelected === null) {
                ...
                const shortenedName =
                    originalName.length > 14
                        ? `${originalName.slice(0, 13)}...${originalName.slice(
                              -8
                          )}`
                        : originalName;
                ...
                for (const file of fileList) {
                    if (
                        !file.name.includes(originalName) &&
                        file.size === sizeFile
                    ) {
